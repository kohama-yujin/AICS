...
class Overlapap:
    ...
    def filter_5ghz(self, df):
        """
        5GHz帯（5000MHz以上）のデータのみ残す
        """
        if "Center Freq(MHz)" not in df.columns:
            print("警告: 'Center Freq(MHz)' カラムがありません。")
            return pd.DataFrame()
        return df[df["Center Freq(MHz)"] >= 5000].copy()
    ...
    def get_weight_and_coords(self, floor, p, l):
        ...
        # 5GHz帯のみフィルタ
        rssi_floor_data = self.filter_5ghz(rssi_floor_data)
