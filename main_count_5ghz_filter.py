...
from count_5ghz_filter import Count5GHzFilter
...
def main():
    ...
    print("=== カウント + 5GHzフィルタ実行 ===")
    print("適用フィルタ: 5GHz + カウント優先（重複許可）")
    
    # カウント + 5GHzフィルタのインスタンス化
    count_5ghz_filter = Count5GHzFilter(ap.data, rssi.data)
    ...
    for p in range(1, 60):  # 位置P=1から59まで
        try:
            weight, coordinate = count_5ghz_filter.get_weight_and_coords(4, p, 3)
            print(f"カウント + 5GHzフィルタ結果 位置P={p}の重み: {weight}, 座標: {coordinate}")
            
            # 重みや座標が空でないかチェック
            if not weight or not coordinate:
                print(f"位置P={p}: アクセスポイントが見つかりませんでした（カウント + 5GHzフィルタ後）")
                continue
            ...
            print(f"カウント + 5GHzフィルタ 位置P={p}: エラーが発生しました - {e}")
            continue
    
    print(f"\nカウント + 5GHzフィルタ処理完了: {len(results)}個の位置で推定が成功しました")
    
    # 推定座標をまとめて出力
    print("\n=== カウント + 5GHzフィルタ推定位置座標一覧 ===")
    for result in results:
        print(f"位置P={result['position']}: {result['estimated_coordinate']}")
    
    # CSVファイルに結果を出力
    csv_filename = "estimation_results_count_5ghz_filter.csv"
